...
function onCreatePost()
    luaDebugMode = true
    if curStage == 'renoback' then
      charJson = json.parse(getTextFromFile('charselect/lcd.json', false))
    else if curStage == 'streetnight' then
      charJson = json.parse(getTextFromFile('charselect/night.json', false))
    else
      charJson = json.parse(getTextFromFile('charselect/default.json', false))
    end
  end
    characters = charJson.characters
    characters[1][2] = boyfriendName
    defaultChar = boyfriendName
    for i = 1, #characters do 
        addCharacterToList(characters[i][2], 'boyfriend')
    end
end
...
function onStartCountdown()
  if not allowCD and not isStoryMode then
    ...
    makeLuaText('instr', '[ACCEPT] to Select Character | [LEFT] or [RIGHT] to Change Character', screenWidth, 0, 700)
    ...
    makeLuaText('pixelNotice', 'I suggest using "Default Character"\nThis is a pixel stage, as they are not supported!', screenWidth, 0, 120)
    setTextAlignment('pixelNotice', 'center')
    setTextSize('pixelNotice', 25)
    setObjectCamera('pixelNotice', 'other')
    addLuaText('pixelNotice')
    screenCenter('pixelNotice', 'x')
    end
...
function tweenScreen()
  ...
  doTweenX('T5.1', 'char', -1000, 1, 'circInOut')
  doTweenAlpha('T5', 'SELECT', 0, 0.8, 'linear')
  doTweenAlpha('T6', 'pixelNotice', 0, 0.8, 'linear')
...
function refreshCharacter() 
    charData = json.parse(getTextFromFile('characters/'..characters[curSelected][2]..'.json', false))
